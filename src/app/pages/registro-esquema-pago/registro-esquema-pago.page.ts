...
export class RegistroEsquemaPagoPage {
  ...
  constructor(private fb: FormBuilder) {
    ...
    this.formularioEsquema.get('tipo')?.valueChanges.subscribe(tipo => {
      if (tipo === 'Cuota fija') {
        this.formularioEsquema.get('montoMensual')?.setValidators([Validators.required, Validators.min(0)]);
        this.formularioEsquema.get('porcentajeGanancia')?.clearValidators();
        this.formularioEsquema.get('porcentajeGanancia')?.setValue('');
      } else if (tipo === 'Porcentaje') {
        this.formularioEsquema.get('porcentajeGanancia')?.setValidators([Validators.required, Validators.min(0), Validators.max(100)]);
      ...
      this.formularioEsquema.get('porcentajeGanancia')?.updateValueAndValidity();
    });
  }
